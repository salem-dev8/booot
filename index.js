...
// للتأكد من أن البوت يعمل على Render
app.get("/", (req, res) => {
...
async function geminiReply(text) {
  try {
    const response = await axios.post(
      `https://generativelanguage.googleapis.com/v1beta/models/gemini-1:generateContent?key=${GEMINI_KEY}`,
      {
        contents: [{ parts: [{ text }] }]
      }
    );

    return response.data?.candidates?.[0]?.content?.[0]?.text || "تم الاستلام، جاري الرد...";
  } catch (err) {
    console.error("Gemini API error:", err.response?.data || err);
